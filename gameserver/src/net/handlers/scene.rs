...
async fn load_scene(
    ...
    for (group_id, group) in &group_config.group_items {
        ...
        for prop in &group.props {
            ...
            let prop_position = Position {
                x: (prop.pos_y * 1000f64) as i32,
                y: (prop.pos_x * 1000f64) as i32 + 60000,
                z: (prop.pos_z * 1000f64) as i32 + 40000,
        ...
        for npc in &group.npcs {
            ...
            let npc_position = Position {
                x: (npc.pos_y * 1000f64) as i32,
                y: (npc.pos_x * 1000f64) as i32,
        ...
        for monster in &group.monsters {
            ...
            let monster_position = Position {
                x: (monster.pos_y * 1000f64) as i32 - 1000,
                y: (monster.pos_x * -1000f64) as i32 - 30000,
                z: (monster.pos_z * 1000f64) as i32 + 24000,
    ...
    // load battle prop xxx entity
    let mut xxx_group = SceneGroupInfo {
        state: 1,
        group_id: 902,
        ..Default::default()
    };
    {
        xxx_group.entity_list.push(SceneEntityInfo {
            entity_id: 1337,
            group_id: 186,
            inst_id: 300001,
            motion: Some(MotionInfo {
    ...
    scene_info.scene_group_list.push(xxx_group);
